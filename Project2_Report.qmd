...
2. Create a new dataset called `dataset3no` that contains only those individuals in `dataset3` who have `mouse_allergic == "yes"`. Using `dataset3no`, fit a linear model with `cough_days` as the outcome and `mouse` as the predictor.
