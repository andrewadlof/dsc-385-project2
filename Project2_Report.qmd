...
2. Fit a linear model with eNO as the outcome nad age as the predictor variable. Overlay the fitted regression line on the scatterplot of eNO and age. How would you characterize the quality of the fit of this model? Do any of the linear regression model assumptions appear to be violated?
...
1. Create a new dataset called `dataset3yes` that contains only those individuals who have `mouse_allergic == "yes"`. Using `dataset3yes`, fit a linear model with `cough_days` as the outcome and `mouse` as the predictor.

2. Create a new dataset called `dataset3no` that contains only those individuals who have `mouse_allergic == "yes"`. Using `dataset3no`, fit a linear model with `cough_days` as the outcome and `mouse` as the predictor.
...
Make a scatterplot of `cough_days` (y-axis) and `mouse` (x-axis), with a separate panel for those who are not mouse allergic and those who are mouse allergic (you can use `facet_wrap()` for this). In each plot, overlay the linear regression line fit using `geom_smooth(method = "lm")`.
...
Make a scatterplot of the residuals versus the fitted values from this model. Describe what you see in the residuals and if there are any violations of the lienar regression model assumptions evident in the plot.
